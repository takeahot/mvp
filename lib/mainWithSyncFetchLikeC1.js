...
class Token {
	...
	getToken() {
		...
		console.log(o);
		let fetchResponse = fetch(o);
		s.c(fetchResponse);
		Object.assign(this,JSON.parse(fetchResponse.body),{requestTime: requestTime});
		return this;
	}
	...
	refreshToken () {
		...
		let requestTime = new Date().getTime();
		Object.assign(this,JSON.parse(fetch(o).body),{requestTime: requestTime});
		return this;
	}

	getValidToken() {
		if (this.access_token) {
			if (this.expires_in*1000 < (new Date().getTime() - this.requestTime)) {
				this.refreshToken();
			}
		} else {
			this.getToken();
