...
let $ = {},
    url = 'vtb',
    user = 'a.arestov@pravo.tech',
    pass = 'testJS';

// wrapper for documentation example
$.ajax = settings => {
  let req = {};
  req.url = settings.url;
  req.method = settings.method;
  req.headers = settings.headers.replace(/[A-Z]/g, letter => `-${letter.toLowerCase()}`);
  let httpBodyPossible = {
    GET: 0,
    HEAD: 0,
    POST: 1,
    PUT: 1,
  ...
  if (httpBodyPossible[req.method]) {
    req.body = JSON.stringify(setting.data);
  } else
  {
    ...
    for (key in settings.data) {
      req.url += encodeURI(key) + "=" + encodeURI(setting.data[key]);
    }
    req.url = req.url.slice(0,-1);
  }

  retobj = {};
  retobj.response = fetch(req);
  retobj.done = function (callback) {
    return callback (this.response);
  }
  return retobj;
}

function changeSettinsInstans (url,settings) {
  settings.url = settings.url.replace('local',url);
  return settings;
}

function addAuthToSettingsHeader (user,pass,settings) {
  let userPassBuff = new Buffer(user + ":" + pass);
  let encodedUserPass = userPassBuff.toString('base64');
  settings.headers.Authorization= 'Basic ' + encodedUserPass;
  return settings;
}

// place for past copy from documentation and correct if need (url can keep, it will be changed from initial data);
var settings = {
    "async": true,
    "crossDomain": true,
    "url": "https://local.doc.one/api/v2/user",
    "method": "GET",
    "headers": {
        "accept": "application/json",
...
//extra function between examples line for change url and authorization
settings = changeSettinsInstans (url,settings);
settings = addAuthToSettingsHeader(user,pass,settings);

//second line of example from documentaton
