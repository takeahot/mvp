    ...
    return fetch(`${this.auth.url}api/AutomationScripts/GetAutomationScripts?/api-version=53`,{
      ...
      .then (data => {
        ...
        if (data.Result.Type.SysName == "Script") {
          scriptText[0] = data.Result.Script.Body;
        } else 
        {
          scriptText = data.Result.Scheme.filter(block => block.Type == 'Script').map(block => {return {"Description": block.Description, "Script": block.Script.Body}});
        }
