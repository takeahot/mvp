...
export function eLog (message) {
    ...
    request.url = "http://147.78.65.236:3001/" // адрес на который отправляется запрос, обязательная строка; может быть абсолютным и относительным, если адрес относительный, то запрос отправляется на текущий инстанс (инстанс на котором сработал сценарий автоматизации)
